...
# -------------------- কনফিগারেশন --------------------
BOT_TOKEN = "8772386853:AAHKdHFfFRU4AgYU7MLu2Y7bXToBH1E4pgo"
ADMIN_ID = 7058712225
...
def get_join_keyboard(file_id_param=None):
    ...
    if "dynamic_channels" in bot_settings:
        for ch_id, ch_info in bot_settings["dynamic_channels"].items():
            markup.add(InlineKeyboardButton(text=f"📢 {ch_info['name']}", url=ch_info["link"]))
        ...
        try:
            ...
            if datetime.now() >= end_time:
                ...
                if participants:
                    ...
                    bot.send_message(ADMIN_ID, f"🎉 **Giveaway Ended!**\n🏆 Winner: `{winner}`\n🎁 Prize: {prize}")
                    try:
                        bot.send_message(winner, f"🎉 **Congratulations!**\nYou won the giveaway!\n🎁 Prize: {prize}\nContact admin to claim.")
                    except:
                        pass
...
def handle_callbacks(call):
    ...
    if call.data == "toggle_force_join":
        ...
        if giveaway.get("active"):
            ...
            status = "🔴 Inactive"
        bot.edit_message_text(f"🎁 **Giveaway Management**\n\nStatus: {status}", chat_id, message_id, parse_mode="Markdown", reply_markup=markup)
        ...
        if giveaway.get("active"):
            ...
            if participants:
                ...
                if "winners" not in giveaway:
                    giveaway["winners"] = []
                giveaway["winners"].append({"user_id": winner, "prize": prize, "time": datetime.now().isoformat()})
                bot.send_message(chat_id, f"🎉 **Giveaway Ended!**\n🏆 Winner: `{winner}`\n🎁 Prize: {prize}")
                try:
                    bot.send_message(winner, f"🎉 **Congratulations!**\nYou won the giveaway!\n🎁 Prize: {prize}\nContact admin to claim.")
                except:
                    pass
        ...
        bot.edit_message_text(f"👑 **Admin Management**\n\n**Current Admins:**\n{admins_list}", chat_id, message_id, parse_mode="Markdown", reply_markup=markup)
        ...
        for key, data in file_database.items():
            ...
            if data.get("is_text_key"):
                info_text = (
                    f"🔑 **Secret Key Panel**\n"
                    f"📛 Name: {data['panel_name']}\n"
                    f"🆔 ID: `{key}`\n"
                    f"🔗 Link: `{share_link}`"
                )
            else:
                file_name = data.get("file_name", f"file_{key[:8]}")
                file_type = data.get("file_type", "Unknown").capitalize()
                info_text = (
                    f"📁 **File ({file_type})**\n"
                    f"📛 Name: `{file_name}`\n"
                    f"🆔 ID: `{key}`\n"
...
def handle_messages(message):
    ...
    if user_states.get(chat_id) == "waiting_for_admin_password":
        password = text.strip()
    ...
    if user_states.get(chat_id) == "waiting_for_admin_password_save":
        password = text.strip()
    ...
    if user_id == ADMIN_ID:
        if user_states.get(chat_id) not in ["waiting_for_admin_password", "waiting_for_admin_password_save"] and not is_admin_logged_in(user_id):
            pass
        ...
        if current_state == "waiting_for_schedule_message":
            try:
                ...
                scheduled_messages.append({"message": msg, "time": schedule_time.isoformat(), "sent": False})
                sync_db()
                user_states[chat_id] = None
        ...
        if current_state == "waiting_for_broadcast":
            ...
            for u_id in list(registered_users):
                if u_id == ADMIN_ID:
                    continue
                try:
                    bot.send_message(u_id, f"📢 **Broadcast Message:**\n\n{text}", parse_mode="Markdown")
                    success_count += 1
                    time.sleep(0.05)
                except Exception as e:
                    logger.error(f"Broadcast failed to {u_id}: {e}")
            bot.edit_message_text(f"✅ Broadcast completed!\n🎯 Sent to `{success_count}` users", chat_id, status_msg.message_id, parse_mode="Markdown")
    ...
    if user_id != ADMIN_ID and not check_user_membership(user_id):
        bot.send_message(chat_id, "🔒 Please join all channels to use this bot!", reply_markup=get_join_keyboard())
        return
    ...
    if text == "🎙️ Voice" or text == "/voice":
        ...
        try:
            ...
            with open(file_path, 'rb') as audio:
                bot.send_audio(chat_id, audio, caption="🎵 Here is your voice message!", reply_markup=get_main_keyboard())

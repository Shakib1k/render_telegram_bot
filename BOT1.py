...
def add_target_group(group_id):
    if group_id not in target_groups:
        ...
        save_db(db)
        logger.info(f"✅ টার্গেট গ্রুপ যোগ করা হয়েছে: {group_id}")
        return True
    return False
...
def remove_target_group(group_id):
    if group_id in target_groups:
        ...
        db["target_groups"] = target_groups
        save_db(db)
        logger.info(f"✅ টার্গেট গ্রুপ বাদ দেওয়া হয়েছে: {group_id}")
        return True
    return False
...
def add_source_group(group_id):
    if group_id not in source_groups:
        ...
        save_db(db)
        logger.info(f"✅ সোর্স গ্রুপ যোগ করা হয়েছে: {group_id}")
        return True
    return False
...
def remove_source_group(group_id):
    if group_id in source_groups:
        ...
        db["source_groups"] = source_groups
        save_db(db)
        logger.info(f"✅ সোর্স গ্রুপ বাদ দেওয়া হয়েছে: {group_id}")
        return True
    return False

# -------------------- ফরওয়ার্ড ফাংশন (আপডেটেড) --------------------
def forward_to_groups(content_type, file_id=None, text=None, caption=None, from_chat_id=None, message_id=None):
    ...
    # ডিবাগ লগ
    logger.info(f"📤 ফরওয়ার্ড শুরু: টাইপ={content_type}, টার্গেট={target_groups}")
    ...
    for group_id in target_groups:
        try:
            logger.info(f"📤 পাঠানোর চেষ্টা: {group_id}")
            
            if from_chat_id and message_id:
                # ফরওয়ার্ড মেসেজ
                sent = bot.forward_message(group_id, from_chat_id, message_id)
                logger.info(f"✅ ফরওয়ার্ড সফল: {group_id}")
            else:
                # সরাসরি ফাইল পাঠান
                ...
                logger.info(f"✅ ফাইল সফল: {group_id}")
            ...
            error_msg = str(e)
            error_messages.append(f"{group_id}: {error_msg}")
            logger.error(f"❌ গ্রুপ {group_id} এ পাঠাতে ব্যর্থ: {e}")

    # রেজাল্ট মেসেজ
    result = f"✅ {success_count} টি গ্রুপে পাঠানো হয়েছে"
    if fail_count > 0:
        result += f"\n❌ {fail_count} টি গ্রুপে ব্যর্থ"
        if error_messages:
            result += "\n\n⚠️ ত্রুটির বিবরণ:\n" + "\n".join(error_messages[:3])
            if len(error_messages) > 3:
                result += f"\n... এবং {len(error_messages)-3} টি বেশি"

    logger.info(f"📊 ফরওয়ার্ড রেজাল্ট: {result}")
    return result
        ...
        try:
            ...
            if add_target_group(group_id):
                try:
                    ...
                    logger.error(f"গ্রুপের নাম পেতে ব্যর্থ: {e}")
                    bot.send_message(
                        message.chat.id,
...
def handle_group_messages(message):
    ...
    # চেক করুন সোর্স গ্রুপ কিনা
    if chat_id not in source_groups:
        return
    
    # সোর্স ফরওয়ার্ড সক্রিয় কিনা
    if not settings.get('forward_from_source', True):
        return
    
    logger.info(f"📥 সোর্স গ্রুপ থেকে মেসেজ: {chat_id}")
    
    # টার্গেট গ্রুপে ফরওয়ার্ড
    for target_id in target_groups:
        if target_id != chat_id:
            try:
                bot.forward_message(target_id, chat_id, message.message_id)
                logger.info(f"✅ অটো-ফরওয়ার্ড সফল: {chat_id} -> {target_id}")
            except Exception as e:
                logger.error(f"❌ অটো-ফরওয়ার্ড ব্যর্থ {chat_id} -> {target_id}: {e}")

def forward_file_from_message(message):
    """মেসেজ থেকে ফাইল ফরওয়ার্ড করে"""
    logger.info(f"📤 ফাইল ফরওয়ার্ড: {message.content_type}")
...
def run_bot():
    try:
        logger.info("🚀 বট পোলিং শুরু হচ্ছে...")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("💡 বট চালু আছে!")
    print("📌 ডিবাগ লগ চালু আছে - সমস্যা হলে লগ চেক করুন")
    print("=" * 60)
    ...
    # Flask অ্যাপ চালান (রেন্ডারের জন্য)

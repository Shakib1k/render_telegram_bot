...
# gtts ইম্পোর্ট (ঐচ্ছিক)
try:
    from gtts import gTTS
    GTTS_AVAILABLE = True
except ImportError:
    GTTS_AVAILABLE = False
    print("gTTS ইনস্টল নেই। ভয়েস জেনারেশন ফিচার কাজ করবে না।")

# Pillow ইম্পোর্ট (ঐচ্ছিক)
try:
    from PIL import Image, ImageDraw, ImageFont
    PIL_AVAILABLE = True
except ImportError:
    PIL_AVAILABLE = False
    print("Pillow ইনস্টল নেই।")

# -------------------- কনফিগারেশন --------------------
BOT_TOKEN = "8896467432:AAH2Y1OT6hg4pGq77Yl3YcMhr_FUAY68NW8"  # আপনার টোকেন দিন
ADMIN_ID = 7058712225
...
def get_default_db():
    ...
    return {
        ...
        "bot_settings": {
            ...
            "welcome_message": "Welcome to Anish Advanced Assistant Bot!",
            "auto_delete": False,
            "delete_after": 60,
            "log_channel": None,
            "auto_reply": {},
...
def send_welcome(message):
    ...
    if user_id == ADMIN_ID and not file_key:
        admin_msg = (
            "╔════════════════════════════╗\n"
            "║  👋 Welcome Admin!\n"
            "║  🛠️ Bot Control Panel\n"
    ...
    if check_user_membership(user_id):
        if file_key:
            ...
            welcome_msg = bot_settings.get("welcome_message", "Welcome to Anish Advanced Assistant Bot!")
            bot.send_message(
                chat_id,
                f"╔════════════════════════════╗\n"
                f"║  👋 {welcome_msg}\n"
    ...
    # কাস্টম কমান্ড ম্যানেজমেন্ট
        ...
        for key, data in file_database.items():
            ...
            if data.get("is_text_key"):
                ...
                info_text = f"📁 **File ({data['file_type'].capitalize()})**\n🆔 ID: `{key}`\n🔗 Link: `{share_link}`"
...
def handle_admin_files(message):
    ...
    if file_id:
        ...
        logger.info(f"ফাইল আপলোড: {unique_key} by admin")
...
def setup_webhook():
    """রেন্ডারে ওয়েবহুক সেটআপ"""
    try:
        # ওয়েবহুক রিমুভ করে পোলিং ব্যবহার
        bot.remove_webhook()
...
if __name__ == "__main__":
    logger.info("🤖 Advanced Bot is starting on Render...")

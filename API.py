...
# এনভায়রনমেন্ট ভেরিয়েবল থেকে টোকেন নিন
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8861425904:AAEFpWoal1ttkS0ot0I1TuVQ_dylSfTZo_Q")
ADMIN_ID = int(os.environ.get("ADMIN_ID", 7058712225))
...
def set_webhook():
    ...
    try:
        ...
        if not webhook_url:
            logger.error("WEBHOOK_URL সেট করা নেই!")
            return False
        
        # ওয়েবহুক সেট করুন
        result = bot.set_webhook(url=webhook_url + "/webhook")
        if result:
            logger.info(f"✅ ওয়েবহুক সেট করা হয়েছে: {webhook_url}/webhook")
            return True
        else:
            logger.error("❌ ওয়েবহুক সেট করতে ব্যর্থ!")
            return False
    except Exception as e:
...
if __name__ == "__main__":
    print("=" * 50)
    print("🤖 মাল্টি-গ্রুপ ফরওয়ার্ডার বট (Render.com)")
    ...
    if webhook_set:
        print("✅ বট চালু আছে! ওয়েবহুক মোডে চলছে...")
        print("📌 বটের URL: " + os.environ.get("WEBHOOK_URL", "সেট করা নেই"))
    else:
        print("⚠️ ওয়েবহুক সেট করা যায়নি! পোলিং মোডে চলছে...")
        # ব্যাকআপ হিসেবে পোলিং
        threading.Thread(target=lambda: bot.infinity_polling(timeout=30)).start()
    
    # Flask সার্ভার চালু করুন
